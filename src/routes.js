...
import { ARBITRUM, AVALANCHE, POLYGON } from './addresses'
...
export default function routes(app) {
  app.get('/api/earn/:account', async (req, res, next) => {
    ...
    const validChainNames = new Set(['arbitrum', 'avalanche', 'polygon'])
  ...
  app.get('/api/candles/:symbol', async (req, res, next) => {
    ...
    const validSources = new Set([ARBITRUM, AVALANCHE, POLYGON])
    if (!validSources.has(preferableChainId)) {
      throw createHttpError(400, `Invalid preferableChainId ${preferableChainId}. Valid options are ${ARBITRUM}, ${AVALANCHE}, ${POLYGON}`)
    }
